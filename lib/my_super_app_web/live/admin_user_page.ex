defmodule MySuperAppWeb.PageAdmin do
  @moduledoc false
  ...
  def mount(_params, session, socket) do
    ...
    operator_id = nil
    role_id = nil
    total_users = CasinosAdmins.count_users(operator_id, role_id)
...

    {
      ...
      assign(
        socket,
        sort: [name: "ASC"],
        users: CasinosAdmins.get_users(1, 10, operator_id, role_id),
    ...
    <div class="container mx-auto p-6 text-black">
      <div class="flex justify-between items-center mb-6">
        <div class="flex gap-4">
          <form phx-change="operator_changed" class="flex items-center">
          <label for="operator" class="mr-2 text-sm font-medium text-blue-600">Operator:</label>
          <select
            id="operator"
            name="operator"
           ...
           {#for operator <- @operators}
             <option value={operator.id} selected={@selected_operator_id == operator.id}>
                {operator.name}
              </option>
            {/for}
          </select>
        ...
        <select
          ...
          {#for role <- Enum.filter(@all_roles, fn role -> role.operator_id == @selected_operator_id end)}
      ...
      <Table {=@sort} items={user <- @users} sorting_click="handle_sorting_click" is_cell_border>
        ...
        <Table.Column name="inserted_at" label="Created at" sortable>
        ...
        <Table.Column name="role_id" label="Role" sortable>
          {CasinosAdmins.get_role!(user.role_id).name}
        </Table.Column>
        <Table.Column name="operator_id" label="Operator" sortable>
  ...
  def handle_event("users_per_page_changed", %{"users_per_page" => users_per_page}, socket) do
    ...
    users = CasinosAdmins.get_users(1, users_per_page, socket.assigns.operator_id, socket.assigns.role_id, socket.assigns.sort_key, socket.assigns.sort_dir)
  ...
  def handle_event("modal_open_edit" <> user_id, _params, socket) do
    ...
    {:noreply,
     assign(socket,
       ...
       selected_role_id: CasinosAdmins.get_role!(selected_user.role_id).name
     )}
  end
  ...
  def handle_event("operator_changed", %{"operator" => operator_id}, socket) do
    ...
    users = CasinosAdmins.get_users(1, socket.assigns.limit, operator_id, socket.assigns.role_id)
